  ...
  >
    ...
    >
      <div class="rounded-sm bg-white shadow-lg">
        ...
        <VForm v-slot="{ errors, isSubmitting, meta, handleSubmit }" ref="formLogIn" as="div">
          <form @submit="handleSubmit($event, enterMember)">
            <div
              v-if="hasAccount"
...
const enterMember = async () => {
  if (hasAccount.value) {
    const { email, password } = tempMember.value
