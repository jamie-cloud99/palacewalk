<template>
  <swiper-container class="mb-10 pb-5" :space-between="24" :slides-per-view="1"
    :breakpoints="{ 768: { slidesPerView: 2 }, 1280: { slidesPerView: 3 } }" ref="swiperEl">
    <swiper-slide>
      <figure class="exhibit-item">
        <img class="w-full h-full object-cover object-centers" :src='`/images/exhibitions/exhibition-U009.jpg`'
          alt="U009">
  ...
  <div class="absolute bottom-0 z-10 right-0 w-full h-15 text-dark">
    <div class="flex justify-between items-center">
      <ul class="flex gap-x-2">
        <li v-for="item in slides.totalSlides - curSlideShowed + 1" :key="'slide' + item" class="bg-dark-400 w-6 h-1"
          :class="{ '!bg-primary': item === slides.curSlide }"></li>
      </ul>
      <div class="flex gap-x-2">
        <button type="button" class="btn text-primary disabled:text-dark-400 py-1 px-2.5 rounded text-xl" @click="goPrev"
          :class="{ 'hover:bg-dark-200': slides.havePrev }" :disabled="!slides.havePrev">
          <i class="fa-solid fa-chevron-left"></i>
        </button>
        <button type="button" class="btn text-primary disabled:text-dark-400 py-1 px-2.5 rounded text-xl" @click="goNext"
          :class="{ 'hover:bg-dark-200': slides.haveNext }" :disabled="!slides.haveNext">
